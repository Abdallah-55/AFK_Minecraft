...
function createBot(config, index) {
  ...
  function antiAFK() {
    if (!bot.entity) {
      console.log(`Bot ${index} entity is not available, skipping antiAFK action.`)
      return
    }
  ...
  bot.on('chat', (username, message) => {
    if (username !== bot.username) {
      const formattedMessage = message.replace(/shards?/gi, (match) => `<span style="color: purple">${match}</span>`)
